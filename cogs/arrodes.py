...
class ArrodesCog(commands.Cog, name="Arrodes"):
    ...
    async def on_message(self, message: discord.Message) -> None:
        """
        Fluxo principal:
        1. '#' → Arrodes acorda e pede a pergunta
    ...
    def _build_embed(
        ...
        embed = discord.Embed(
            ...
            color=discord.Color.from_rgb(148, 103, 189),  # roxo místico
        )
        embed.add_field(
            name="📜 Pergunta",
            value=question[:1024],
        ...
        # Se a resposta for muito longa, adicionar continuação
        if len(answer) > 1024:
            embed.add_field(
                name="✨ Continuação",
